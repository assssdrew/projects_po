...
4. Сверь номер версии с тем, что в этом README (сейчас `6.6`) — если меньше,
...
- **«Не успели обновить за 20с»**: в Scriptable параллельные `Request.loadJSON`
  ...
  запрашиваются **по очереди**, каждый URL жёстко ограничен `withTimeout(4с)`.
  Добавлены `.min.json` зеркала fawaz. Если сеть не ответила — показывается
  понятное сообщение с возрастом кэша, а не общий таймаут.
