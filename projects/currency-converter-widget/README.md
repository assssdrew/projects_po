...
4. Сверь номер версии с тем, что в этом README (сейчас `6.3`) — если меньше,
...
- Один и тот же список валют, что и в полноэкранной таблице:
  **флаг+код** слева, **сумма** справа, строки разделены тонкой полоской
- Активная валюта (та, в которую введена сумма) подсвечена синим —
  ровно как в полноэкранной таблице
- Никакой служебной метки версии/времени на самой плитке — только курсы и
  значок обновления в углу
- На каждый размер видно столько строк, сколько влезает: `small` — 3,
  `medium` — 5, `large` — 7 (или меньше, если валют в списке не хватает)

### Тап по плитке

- Тап по плитке целиком (на любом размере) сразу открывает полноэкранную
  таблицу всех валют с клавиатурой — см. «v6.2» и «v6.3» выше
- Тап по значку обновления в правом нижнем углу (на `medium`/`large`) сразу
  форсирует обновление курсов, без открытия таблицы. На `small`-виджете
  своей тап-зоны у значка нет (у iOS такой размер поддерживает только один
  общий тап-таргет) — там любой тап открывает таблицу
...
- Значок обновления: зелёный, если курсы получены из сети меньше часа назад,
  тусклый — если показывается более старый кэш. Никакого текста рядом с ним
  на самой плитке (версия видна только в заголовке меню при ручном ▶ Play,
  см. ниже)
...
ручной ▶ Play ведут в разные места, см. «Поведение (v6.3)» выше).
