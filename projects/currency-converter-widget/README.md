...
4. Сверь номер версии с тем, что в этом README (сейчас `6.1`) — если меньше,
...
## Поведение (v6) — 1 стиль виджета

Референс-мокап лежит в `mockups/minimal.png` (исходник — `mockups/minimal.html`).
Это не художественная иллюстрация, а прямой HTML/CSS-рендер с теми же
цветами, шрифтами, фиксированными ширинами колонок и текстом, что и в
`CurrencyConverterCore.js` — так что "образец" и реальный код по построению
не могут разойтись в вёрстке (могут отличаться разве что живые курсы/
проценты, потому что в мокапе они захардкожены как пример).

<img src="mockups/minimal.png" alt="Минимальный стиль — мокап" width="480" />

- Сплошной тёмный фон `#121316`, без градиентов и карточек
- Плоский список `147 USD → 12 241,3 RUB`, под каждой строкой — изменение
  курса за сутки `▲0.4%  за 24ч` и рядом мини-график сглаженной кривой
  (зелёным — рост, красным — падение), если накопилась история
- Никакой служебной метки версии/времени на самой плитке — только курсы и
  значок обновления в углу
- Компактный и предсказуемый по вёрстке на любом размере виджета
...
- Значок обновления: зелёный, если курсы получены из сети меньше часа назад,
  ...
  на самой плитке (версия/время видны только в заголовке меню, см. ниже)
- Тап по виджету (или по скрипту в приложении Scriptable) открывает меню:
  «Задать сумму для виджета», «Конвертировать сумму», «Таблица всех валют»,
  «Настроить пары для виджета», «Валюты для таблицы», «Обновить курсы
  сейчас». В заголовке меню — текущий `CORE_VERSION`, для диагностики того,
  подтянулось ли обновление кода
...
- «Таблица всех валют» — полноэкранная интерактивная таблица с клавиатурой,
  подробнее см. «v6.2» выше
...
- История курсов (для индикатора изменения и спарклайна) копится локально на
  устройстве, не чаще одной точки в 15 минут, хранится до ~60 последних точек
