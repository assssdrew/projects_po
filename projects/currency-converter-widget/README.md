...
Ниже для каждого стиля — референс-мокап, лежит рядом в `mockups/`. Это не
художественная иллюстрация, а прямой HTML/CSS-рендер (исходник — соседний
`.html`-файл) с теми же цветами, шрифтами, фиксированными ширинами колонок
и текстом, что и в `CurrencyConverterCore.js` — так что "образец" и
реальный код по построению не могут разойтись в вёрстке (могут отличаться
разве что живые курсы/проценты, потому что в мокапе они захардкожены как
пример).
