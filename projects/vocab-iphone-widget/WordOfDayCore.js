// WordOfDayCore — v29: 3 слота по 3 строки, отступы 14 pt
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V29";
...
/** Ширина текста: берём максимум из DrawContext и оценки — иначе перенос не срабатывает. */
function measureTextWidth(text, fontSize, bold) {
  const s = String(text || "");
  if (!s) return 0;
  // кириллица чуть шире латиницы — оценка с запасом
  const approx = s.length * fontSize * (bold ? 0.6 : 0.56);
  let exact = 0;
  try {
    ...
    if (size && size.width) exact = size.width;
  } catch (e) {}
  return Math.max(exact, approx);
}
...
/**
 * Один пример = слот на 3 строки: EN + перевод (до 2 строк).
 */
function addExampleBlock(w, word, opts) {
  ...
  const accessory = !!opts.accessory;
  const slotW = opts.slotWidth || contentWidth();
  const slotH = opts.slotHeight || Math.ceil(enSize + 3 + ruSize * 2);
  const sentence = (word && word.ex) || (word && word.en) || "";
  const ruText = (word && word.exRu) || (word && word.ru) || "";

  if (accessory) {
    addHighlightedLine(w, sentence, word.en, enSize, true, slotW);
    w.addSpacer(1);
    const ru = leftText(w.addText(ruText));
    ru.font = Font.systemFont(ruSize);
    ru.textColor = ruColor(ruAlpha);
    ru.lineLimit = 2;
    ru.minimumScaleFactor = 0.8;
    return;
  }

  const slot = w.addStack();
  slot.layoutVertically();
  slot.size = new Size(slotW, slotH);
  try {
    slot.topAlignContent();
  } catch (e) {}
  try {
    slot.spacing = 3;
  } catch (e) {}

  addHighlightedLine(slot, sentence, word.en, enSize, false, slotW);

  const wrapped = wrapByWidth(ruText, ruSize, slotW * 0.92, 2);
  const ru = leftText(slot.addText(wrapped));
  ru.font = Font.systemFont(ruSize);
  ru.textColor = ruColor(ruAlpha);
  ru.lineLimit = 2;
...
function createWidget(progress) {
  ...
  let enSize = progress.kind === "rule" ? 18 : 14;
  let ruSize = progress.kind === "rule" ? 15 : 11;
  let blockGap = progress.kind === "rule" ? 14 : 8;
  ...
  if (family === "large") {
    ...
    enSize = progress.kind === "rule" ? 16 : 14;
  ...
  if (progress.kind === "rule" && progress.rule) {
    ...
    if (family === "accessoryCircular") {
      ...
      // 3 равных слота: у каждого место на EN + 2 строки перевода
      const items = [main].concat(related);
      const n = items.length;
      const slotW = contentWidth();
      const innerH = Math.max(60, widgetOuterHeight() - padY * 2);
      const gaps = Math.max(0, n - 1);
      const minGap = accessory ? 2 : blockGap;
      // целевая высота слота под 3 строки
      const slotNeed = Math.ceil(enSize * 1.1 + 2 + ruSize * 1.15 * 2);
      let gap = minGap;
      let slotH = Math.floor((innerH - gap * gaps) / Math.max(1, n));
      if (slotH < slotNeed && gaps > 0) {
        const tightGap = Math.floor((innerH - slotNeed * n) / gaps);
        if (tightGap >= 4) {
          gap = tightGap;
          slotH = slotNeed;
        } else {
          gap = 4;
          slotH = Math.floor((innerH - gap * gaps) / Math.max(1, n));
        }
      }
      // слот не больше доступной доли — иначе обрежет крайние строки
      const maxSlot = Math.floor((innerH - gap * gaps) / Math.max(1, n));
      slotH = Math.min(slotH, maxSlot);

      for (let i = 0; i < n; i++) {
        if (i > 0) w.addSpacer(gap);
        addExampleBlock(w, items[i], {
          enSize: enSize,
          ruSize: ruSize,
