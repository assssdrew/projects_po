...
const SCRIPT_NAME = "WordOfDay"; // имя скрипта в Scriptable
...
function loadProgress() {
  ...
  if (!fm.fileExists(path)) {
    return { index: 0, current: null, history: {} };
  }
  try {
...
function isComplete(item) {
  return !!(item && item.seen && item.learned && item.reviewed);
}
...
function ensureCurrentWord(progress) {
  if (!progress.current) return nextWord(progress);
  const key = progress.current.en.toLowerCase();
  if (isComplete(progress.history[key])) return nextWord(progress);
  return progress;
}

function mark(progress, field) {
  const key = progress.current.en.toLowerCase();
  if (!progress.history[key]) {
    progress.history[key] = {
  ...
  progress.history[key][field + "At"] = new Date().toISOString();
  saveProgress(progress);
  return progress;
}
...
function getAction() {
  try {
    if (args.queryParameters && args.queryParameters.action) {
      return String(args.queryParameters.action);
    }
  } catch (e) {}
  try {
    if (args.plainTexts && args.plainTexts.length) {
      return String(args.plainTexts[0]);
    }
  } catch (e) {}
  return null;
}
...
function addButton(row, label, action, active) {
...
function createWidget(progress) {
  ...
  w.backgroundColor = new Color("#0F2744");
  w.setPadding(12, 14, 12, 14);
  // не ставим url на весь виджет — только на кнопки

  const family = config.widgetFamily || "medium";
  const key = progress.current.en.toLowerCase();
  const item = progress.history[key] || {};

  const title = w.addText("СЛОВО");
  title.font = Font.boldSystemFont(10);
  title.textColor = new Color("#8FB3D9");

  w.addSpacer(4);

  const en = w.addText(progress.current.en);
  en.font = Font.boldSystemFont(family === "large" ? 22 : 17);
  ...
  if (family === "small") {
    addButton(row, "🔊", "speak", false);
    row.addSpacer(6);
    addButton(row, "👁", "seen", !!item.seen);
  } else {
    addButton(row, "🔊", "speak", false);
    row.addSpacer(6);
    addButton(row, "👁", "seen", !!item.seen);
    row.addSpacer(6);
    addButton(row, "✅", "learned", !!item.learned);
    row.addSpacer(6);
    addButton(row, "🔁", "reviewed", !!item.reviewed);
  }

  w.addSpacer(6);
  const left = Math.max(0, WORDS.length - (progress.index || 0));
  const foot = w.addText(statusLine(item) + "  ·  ~" + left);
...
function applyMarks(progress, action) {
  if (action === "seen") {
    mark(progress, "seen");
...
async function handleWidgetAction(action, progress) {
  await ensurePhonetic(progress);
  const before = progress.current.en;

  if (action === "speak") {
    await speakWord(progress.current.en);
    notify("Произношение", before);
  } else if (action === "seen" || action === "learned" || action === "reviewed") {
    applyMarks(progress, action);
    const key = progress.current.en.toLowerCase();
    const item = progress.history[key];
    if (isComplete(item)) {
      nextWord(progress);
      await ensurePhonetic(progress);
      notify("Следующее слово", progress.current.en + " — " + progress.current.ru);
    } else {
      const miss = [];
      if (!item.seen) miss.push("👁");
      if (!item.learned) miss.push("✅");
      if (!item.reviewed) miss.push("🔁");
      notify("Отмечено", before + " · ещё: " + miss.join(" "));
    }
  }

  const fresh = ensureCurrentWord(loadProgress());
  Script.setWidget(createWidget(fresh));
}
...
async function runMenu(progress) {
  ...
  alert.message =
    progress.current.ru +
    phoneticLine +
    "\n\nУдобнее жать кнопки на виджете.\nЗдесь — запасное меню.";
  ...
  if (choice === 0) {
    ...
    await handleWidgetAction("seen", progress);
  } else if (choice === 2) {
    await handleWidgetAction("learned", progress);
  } else if (choice === 3) {
    await handleWidgetAction("reviewed", progress);
  } else if (choice === 4) {
    await showHistory(progress);
  } else if (choice === 5) {
    const conf = new Alert();
    conf.title = "Сбросить прогресс?";
    conf.message = "История и очередь обнулятся.";
    conf.addDestructiveAction("Сбросить");
    conf.addCancelAction("Отмена");
    if ((await conf.presentAlert()) === 0) {
      saveProgress({ index: 0, current: null, history: {} });
    }
  }
}
...
async function main() {
  ...
  if (action) {
    // Тап по кнопке на виджете — без большого меню
    await handleWidgetAction(action, progress);
    return;
  }

  // Запуск из Scriptable (Play) — запасное меню
