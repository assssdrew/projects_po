// WordOfDayCore — v9: перенос по ширине medium (329pt), отступы 14/16
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V9";
const RELATED_LIMIT = 2;

/** Ширина виджета Scriptable на iPhone (pt), approx. */
const WIDGET_WIDTH = {
  small: 155,
  medium: 329,
  large: 329,
  accessoryRectangular: 150,
  accessoryInline: 200,
  accessoryCircular: 60,
};
...
function contentWidth() {
  ...
  const full = WIDGET_WIDTH[family] || WIDGET_WIDTH.medium;
  // горизонтальные padding 16+16
  return full - 32;
}

/** Оценка ширины текста в pt (SF / system). */
function estimateWidth(text, fontSize, bold) {
  const s = String(text || "");
  let w = 0;
  for (let i = 0; i < s.length; i++) {
    ...
    let factor;
    if (ch === " " || ch === "-") factor = 0.33;
    else if (code < 128) {
      // латиница / цифры
      if ("ilI1|'.,:;!".indexOf(ch) !== -1) factor = 0.35;
      else if ("mwMW@".indexOf(ch) !== -1) factor = 0.85;
      else factor = bold ? 0.62 : 0.55;
    } else {
      // кириллица и прочее — чуть шире
      factor = bold ? 0.7 : 0.62;
    }
    w += fontSize * factor;
  }
  return w;
...
/** Перенос перевода только если EN + " - " + RU не влезают в ширину виджета. */
function needsWrap(word, enSize, ruSize) {
  const max = contentWidth();
  const line =
    estimateWidth(word.en, enSize, true) +
    estimateWidth(" - ", ruSize, false) +
    estimateWidth(word.ru, ruSize, false);
  return line > max;
}
...
function addWordBlock(w, word, opts) {
  ...
  if (wrap) {
    ...
    en.lineLimit = 3;
    en.minimumScaleFactor = 1;
    addShadow(en, 0.5);

    w.addSpacer(2);
    const ru = w.addText(word.ru);
    ru.font = Font.systemFont(ruSize);
    ru.textColor = new Color("#FFFFFF", ruAlpha);
    ru.lineLimit = 3;
  ...
  sep.textColor = new Color("#FFFFFF", 0.7);
  sep.minimumScaleFactor = 1;
  addShadow(sep, 0.35);

  const ru = row.addText(word.ru);
  ru.font = Font.systemFont(ruSize);
  ru.textColor = new Color("#FFFFFF", ruAlpha);
  ru.lineLimit = 1;
  ru.minimumScaleFactor = 1;
...
function createWidget(progress) {
  ...
  const blockGap = family === "small" ? 10 : 14;
...
async function runMenu(progress) {
  ...
  alert.title = "Word";
  alert.message = lines + "\n\nСмена каждые " + ROTATE_SECONDS + " сек";
