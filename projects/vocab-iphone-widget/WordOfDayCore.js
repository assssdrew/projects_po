// WordOfDayCore — v27: перевод переносится на 2-ю строку, без «…»
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V27";
 ...
 * Длинный перевод переносится на 2-ю строку (без уменьшения шрифта).
 */
function addExampleBlock(w, word, opts) {
  ...
  try {
    block.spacing = accessory ? 1 : 3;
  } catch (e) {}

  addHighlightedLine(block, sentence, word.en, enSize, accessory);

  const ru = leftText(block.addText(ruText));
  ru.font = Font.systemFont(ruSize);
  ru.textColor = ruColor(ruAlpha);
  // 2 строки: перенос вместо «…»; шрифт не уменьшаем
...
function createWidget(progress) {
  ...
  if (accessory) {
    ...
    // 14 pt сверху и снизу, чтобы крайние строки не резались скруглением
    w.setPadding(14, 12, 14, 12);
  }
  ...
  let enSize = progress.kind === "rule" ? 18 : 17;
  let ruSize = progress.kind === "rule" ? 15 : 14;
  let blockGap = progress.kind === "rule" ? 14 : 12;
  let relLimit = RELATED_LIMIT;

  if (family === "large") {
    enSize = progress.kind === "rule" ? 24 : 20;
    ruSize = progress.kind === "rule" ? 20 : 16;
    blockGap = 12;
  } else if (family === "small") {
    enSize = progress.kind === "rule" ? 16 : 15;
    ruSize = progress.kind === "rule" ? 14 : 13;
    blockGap = 8;
    relLimit = 1;
  ...
  if (progress.kind === "rule" && progress.rule) {
    ...
    if (family === "accessoryCircular") {
      ...
      for (const r of related) {
        // home: гибкий spacer — остаток высоты между парами, не сверху/снизу
        if (accessory) w.addSpacer(blockGap);
        else {
