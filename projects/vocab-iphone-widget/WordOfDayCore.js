// WordOfDayCore — v36: зазоры по насыщенности карточки (коротко — раздвинуть)
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V36";
...
/**
 * Насыщенность набора: 0 — все переводы в 1 строку, 1 — все в 2 строки.
 * По ней регулируем зазоры между примерами.
 */
function exampleSaturation(items, ruSize, maxW, accessory) {
  if (!items || !items.length) return 0;
  let extra = 0;
  for (let i = 0; i < items.length; i++) {
    const n = ruLinesFor(items[i], ruSize, maxW, accessory).length;
    extra += Math.max(0, n - 1);
  }
  return extra / items.length;
}

/** Зазор между блоками: редко → шире, плотно → уже. */
...
function createWidget(progress) {
  ...
  let blockGap = progress.kind === "rule" ? 14 : 4;
  ...
  if (progress.kind === "rule" && progress.rule) {
    ...
    if (family === "accessoryCircular") {
      ...
      // Три примера: зазоры от насыщенности (короткие → раздвинуть, длинные → уплотнить)
      ...
      // мало текста — flex между блоками равномерно заполняет высоту;
      // много переносов — фиксированный узкий зазор, чтобы влезло
      ...
      // низ: при spreadFlex делит свободное место с межблочными spacer’ами
      if (!accessory) w.addSpacer();
      return w;
    }
  }

  // правила / accessory: свободная высота вниз
  if (!accessory) w.addSpacer();
  return w;
}
