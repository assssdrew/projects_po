...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V1";
...
function nextRefreshDate(progress) {
  ...
  // минимум через 15 минут, чтобы iOS мог обновлять
...
function createWidget(progress) {
  ...
  w.backgroundColor = new Date().getHours() >= 20 || new Date().getHours() < 7
    ? new Color("#0B1A2B")
    : new Color("#0F2744");
  w.setPadding(14, 16, 14, 16);
  // Без url и без кнопок — тап только обновит виджет (откроет Scriptable на секунду)
  ...
  en.font = Font.boldSystemFont(family === "large" ? 26 : family === "small" ? 16 : 20);
  en.textColor = Color.white();
  en.lineLimit = family === "small" ? 3 : 2;
  en.minimumScaleFactor = 0.65;

  if (item.phonetic) {
    w.addSpacer(4);
    const ph = w.addText(item.phonetic);
    ph.font = Font.systemFont(family === "small" ? 11 : 13);
  ...
  if (family !== "small") {
    ...
    ru.font = Font.systemFont(family === "large" ? 16 : 14);
...
async function showHistory(progress) {
  const entries = Object.values(progress.history).sort((a, b) =>
    (b.lastShownAt || b.firstDay || "").localeCompare(a.lastShownAt || a.firstDay || "")
  );
...
async function runMenu(progress) {
  ...
  alert.message =
    progress.current.ru +
    phoneticLine +
    "\n\nВиджет пассивный: слово само меняется каждые " +
    ROTATE_HOURS +
    " ч.\nСюда заходи редко.";
...
async function main() {
  ...
  // Play / редкий тап — короткое меню (учёба на виджете без нажатий)
