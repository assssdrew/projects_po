// WordOfDayCore — v28: перевод с переносом \n, без гибких spacer'ов
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V28";
 ...
 * Длинный перевод сам переносится на 2-ю строку (без «…»).
 */
function addExampleBlock(w, word, opts) {
  ...
  // EN и RU прямо в виджет: вложенный stack в Scriptable часто ломает перенос
  addHighlightedLine(w, sentence, word.en, enSize, accessory);

  w.addSpacer(accessory ? 1 : 3);
  const wrapped = accessory
    ? ruText
    : wrapByWidth(ruText, ruSize, contentWidth() * 0.98, 2);
  const ru = leftText(w.addText(wrapped));
  ...
  if (progress.kind === "rule" && progress.rule) {
    ...
    if (family === "accessoryCircular") {
      ...
      for (const r of related) {
        // только фиксированный зазор: гибкий spacer съедал высоту и резал перевод «…»
