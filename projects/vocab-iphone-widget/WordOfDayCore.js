// WordOfDayCore — v42: interview без перевода, кегль +2 pt, сверху 12 pt
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V42";
...
function interviewContentHeight(family) {
  ...
  // large: pad top/bottom по 12 pt
  if (f === "large" || f === "extraLarge") return Math.max(120, h - 24);
...
/**
 * Подбирает кегль: вопрос + ответ (без qRu), без обрезки по высоте large.
 */
function fitInterviewLayout(card, family) {
  ...
  let qSize = family === "extraLarge" ? 20 : 19;
  let aSize = family === "extraLarge" ? 15 : 14;
  if (aLen > 900) {
    qSize = 16;
    aSize = 12;
  } else if (aLen > 500) {
  ...
  for (let attempt = 0; attempt < 32; attempt++) {
    ...
    else if (sectionGap > 2) sectionGap -= 1;
    else break;
  }
...
function addInterviewBlock(w, card, family) {
  ...
  // свободное место только снизу — ответ ближе к вопросу
  w.addSpacer();
}
