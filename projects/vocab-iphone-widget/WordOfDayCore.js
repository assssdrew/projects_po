// WordOfDayCore — v15: полный проход 948 слов без повторов
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V15";
const RELATED_LIMIT = 2;
const VOCAB_SIZE = 948; // для проверки полной загрузки
...
function ensureOrder(progress) {
  ...
  // старый прогресс / битая очередь — новый полный проход
...
/**
 * Следующий набор: ровно N слов из очереди полного прохода.
 * Пока круг не закончен — повторов нет. Потом новый shuffle.
 */
function nextWord(progress) {
  ...
  // на всякий случай не даём pos уехать за длину
...
function ensureCurrentWord(progress) {
  ...
  // если на диске старый set без order — всё равно нормализуем очередь
  ensureOrder(progress);
  return progress;
}
...
function passStats(progress) {
  ...
  return { n: n, pos: pos, unique: unique, pass: pass };
}
...
async function runMenu(progress) {
  ...
  alert.message =
    ...
    "\nunique seen: " +
    st.unique +
    "\nTap = next set (no repeats in pass)";
...
module.exports = { main, PASSIVE_MARKER, VOCAB_SIZE };
