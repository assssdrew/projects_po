// WordOfDayCore — v6: 3 строки EN - RU (жирный EN)
...
function msLeft(progress) {
  const shownAt = progress.shownAt || nowMs();
  return Math.max(0, shownAt + rotateMs() - nowMs());
}

function nextRefreshDate(progress) {
  const left = msLeft(progress);
  const step = left > 60 * 60 * 1000 ? 10 * 60 * 1000 : 5 * 60 * 1000;
  const due = nowMs() + Math.min(step, Math.max(60 * 1000, left || step));
  return new Date(due);
}
...
function footerLabel(progress) {
  const num =
    progress.current && typeof progress.current.index === "number"
      ...
      : 1;
  return num + "/" + WORDS.length + "  |  " + countdownLabel(progress);
}
...
/** Одна строка: жирный EN + обычный " - " + RU */
function addWordRow(w, word, opts) {
  ...
  en.font = Font.boldSystemFont(opts.enSize);
  ...
  sep.font = Font.systemFont(opts.ruSize);
  sep.textColor = new Color("#FFFFFF", 0.7);
  addShadow(sep, 0.35);

  const ru = row.addText(word.ru);
  ru.font = Font.systemFont(opts.ruSize);
  ru.textColor = new Color("#FFFFFF", opts.ruAlpha);
  ru.lineLimit = 1;
  ru.minimumScaleFactor = 0.55;
  addShadow(ru, 0.4);

  // чтобы длинные строки сжимались внутри стека
  en.shrinkWrap = false;
  ru.shrinkWrap = false;
}
