// WordOfDayCore — v14: уникальные наборы без пересечения
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V14";
...
function loadProgress() {
  ...
  try {
    ...
    if (!Array.isArray(data.lastSet)) data.lastSet = [];
    return data;
  } catch (e) {
...
/** Следующий индекс очереди, пропуская exclude. */
function pickQueueIndex(startIdx, exclude) {
  const n = WORDS.length;
  if (!n) return 0;
  let idx = ((typeof startIdx === "number" ? startIdx : 0) % n + n) % n;
  for (let step = 0; step < n; step++) {
    const w = WORDS[idx];
    if (!exclude[wordKey(w)]) return idx;
    idx = (idx + 1) % n;
  }
  // все слова в exclude — начинаем круг заново
  return ((typeof startIdx === "number" ? startIdx : 0) % n + n) % n;
}
...
/**
 * Собирает набор: главное + связанные, без повторов внутри
 * и без пересечения с previousKeys (прошлый набор).
 */
function buildSet(progress, size, previousKeys) {
  size = size || 1 + RELATED_LIMIT;
  const exclude = excludeMap(previousKeys);
  const n = WORDS.length;
  let start = typeof progress.index === "number" ? progress.index : 0;
  if (start >= n) start = 0;

  // если прошлый набор почти весь словарь — не блокируем
  if (Object.keys(exclude).length >= n - 1) {
    for (const k of Object.keys(exclude)) delete exclude[k];
  }

  const primaryIdx = pickQueueIndex(start, exclude);
  const primary = asSetItem(WORDS[primaryIdx], primaryIdx);
  exclude[wordKey(primary)] = 1;

  const related = findRelated(primary, Math.max(0, size - 1), exclude);
  for (const r of related) exclude[wordKey(r)] = 1;

  const set = [primary].concat(related);
  // добираем из очереди вперёд, без пересечений
  let cursor = (primaryIdx + 1) % n;
  let guard = 0;
  while (set.length < size && guard < n) {
    guard++;
    const idx = pickQueueIndex(cursor, exclude);
    const w = asSetItem(WORDS[idx], idx);
    const k = wordKey(w);
    if (exclude[k]) break;
    exclude[k] = 1;
    set.push(w);
  ...
  progress.current = primary;
  progress.set = set;
  // очередь уходит за пределы набора, чтобы следующий primary не брал из текущего
  let nextIdx = primaryIdx;
  for (const item of set) {
    if (typeof item.index === "number" && item.index >= nextIdx) {
      nextIdx = item.index + 1;
    }
  }
  if (nextIdx >= n) nextIdx = 0;
  // если nextIdx попал в текущий набор — сдвинуть дальше
  const setKeys = excludeMap(set.map(wordKey));
  nextIdx = pickQueueIndex(nextIdx, setKeys);
  progress.index = nextIdx;
  progress.shownAt = nowMs();

  for (const item of set) recordHistory(progress, item);
  return progress;
}

function nextWord(progress) {
  const prevKeys =
    progress.set && progress.set.length
      ? progress.set.map(wordKey)
      : progress.lastSet || [];
  progress.lastSet = prevKeys.slice();
  const size = 1 + RELATED_LIMIT;
  buildSet(progress, size, progress.lastSet);
  saveProgress(progress);
  return progress;
}

function needsRotate(progress) {
  if (!progress.current || !progress.set || !progress.set.length) return true;
...
function currentSet(progress, limit) {
  const size = typeof limit === "number" ? limit : 1 + RELATED_LIMIT;
  if (progress.set && progress.set.length) {
    return progress.set.slice(0, size);
  }
  if (!progress.current) return [];
  const rest = findRelated(
    progress.current,
    Math.max(0, size - 1),
    excludeMap([wordKey(progress.current)].concat(progress.lastSet || []))
  );
  return [progress.current].concat(rest).slice(0, size);
}
...
async function runMenu(progress) {
  const set = currentSet(progress, 1 + RELATED_LIMIT);
  const lines = set.map((w) => w.en + " - " + w.ru).join("\n");

  const alert = new Alert();
  alert.title = "Word";
  alert.message =
    lines +
    "\n\nTap widget = next unique set\nAuto ~" +
    ROTATE_SECONDS +
    "s (iOS may delay)";
