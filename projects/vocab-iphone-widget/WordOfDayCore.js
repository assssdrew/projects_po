// WordOfDayCore — v40: interview Q&A из interview_qa.json (Play подтягивает файл)
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V40";
...
function addInterviewBlock(w, card, family) {
  ...
  const a = (card && card.a) || "";
  const longA = a.length > 220;
  const qSize = family === "extraLarge" ? (longA ? 18 : 22) : longA ? 17 : 20;
  const aSize = family === "extraLarge" ? (longA ? 13 : 16) : longA ? 12 : 15;
  const ruSize = Math.max(11, aSize - 3);

  const qLines = wrapLines(q, qSize, Math.max(40, maxW - 2), 4);
  const aLines = wrapLines(a, aSize, Math.max(40, maxW - 2), longA ? 8 : 6);
  const ruLines = qRu
