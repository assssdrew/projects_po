// WordOfDayCore — пассивный виджет: слово + пример предложения
...
/** Сколько часов одно слово висит на виджете */
const ROTATE_HOURS = 4;
const PASSIVE_MARKER = "PASSIVE_WIDGET_V2";
...
function fallbackExample(en) {
  const p = cleanPhrase(en);
  if (!p) return "";
  // Уже похоже на готовое предложение / реплику
  if (
    /[.!?]$/.test(p) ||
    /^(I |I'm |I'd |I've |It |It's |We |You |She |He |They |Don't |That's |Have you|Why |Where |What )/i.test(
      p
    )
  ) {
    return p.charAt(0).toUpperCase() + p.slice(1);
  }
  // Фраза из нескольких слов — чаще глагольная
  if (/\s/.test(p)) {
    return "I need to " + p + ".";
  }
  // Одно слово
  return "I want to use the word \"" + p + "\" more often.";
}
...
function nextWord(progress) {
  ...
  // при новом показе можно подтянуть свежий пример, если раньше не было
...
function nextRefreshDate(progress) {
  const shownAt = progress.shownAt || nowMs();
  const due = shownAt + rotateMs();
  const min = nowMs() + 15 * 60 * 1000;
  return new Date(Math.max(due, min));
}

function hoursLeftLabel(progress) {
  const shownAt = progress.shownAt || nowMs();
  const leftMs = Math.max(0, shownAt + rotateMs() - nowMs());
  const h = Math.ceil(leftMs / (60 * 60 * 1000));
  if (h <= 0) return "скоро смена";
  if (h === 1) return "~1 ч";
  return "~" + h + " ч";
}
...
function pickExampleFromEntry(entry) {
  if (!entry || !entry.meanings) return null;
  for (const meaning of entry.meanings) {
    if (!meaning.definitions) continue;
    for (const def of meaning.definitions) {
      if (def && def.example && String(def.example).trim()) {
        let ex = String(def.example).trim();
        if (!/[.!?]$/.test(ex)) ex += ".";
...
function pickPhoneticFromEntry(entry) {
...
async function fetchWordMeta(en) {
  ...
  if (first && !queries.some((q) => q.toLowerCase() === first.toLowerCase())) {
    queries.push(first);
  }
  ...
  for (const q of queries) {
    try {
      ...
      const entry = data[0];
      if (!phonetic) phonetic = pickPhoneticFromEntry(entry);
      if (!example) example = pickExampleFromEntry(entry);
      if (phonetic && example) break;
    } catch (e) {}
  }

  if (!example) example = fallbackExample(en);
  return { phonetic, example };
}

async function ensureWordMeta(progress) {
  if (!progress.current) return null;
  const key = progress.current.en.toLowerCase();
  let item = progress.history[key];
  if (!item) {
    item = {
      en: progress.current.en,
      ru: progress.current.ru,
      firstDay: todayKey(),
      phonetic: null,
      example: null,
      shownCount: 1,
    };
    progress.history[key] = item;
  }

  const needPh = !item.phonetic;
  const needEx = !item.example;
  if (!needPh && !needEx) return item;

  const meta = await fetchWordMeta(progress.current.en);
  if (needPh && meta.phonetic) item.phonetic = meta.phonetic;
  if (needEx) item.example = meta.example || fallbackExample(progress.current.en);
  saveProgress(progress);
  return item;
}
...
function createWidget(progress) {
  ...
  w.backgroundColor =
    hour >= 20 || hour < 7 ? new Color("#0B1A2B") : new Color("#0F2744");
  ...
  const enSize = family === "large" ? 34 : family === "small" ? 22 : 28;
  const phSize = family === "large" ? 16 : family === "small" ? 13 : 15;
  const ruSize = family === "large" ? 20 : family === "small" ? 14 : 17;
  const exSize = family === "large" ? 15 : family === "small" ? 12 : 14;

  const en = w.addText(progress.current.en);
  en.font = Font.boldSystemFont(enSize);
  en.textColor = Color.white();
  en.lineLimit = family === "small" ? 2 : 2;
  en.minimumScaleFactor = 0.55;

  if (item.phonetic) {
    w.addSpacer(3);
    const ph = w.addText(item.phonetic);
    ph.font = Font.systemFont(phSize);
    ph.textColor = new Color("#A8C5E0");
    ph.lineLimit = 1;
  }

  w.addSpacer(family === "small" ? 4 : 6);
  const ru = w.addText(progress.current.ru);
  ru.font = Font.systemFont(ruSize);
  ru.textColor = new Color("#D7E6F5");
  ru.lineLimit = family === "small" ? 2 : 2;
  ru.minimumScaleFactor = 0.7;

  const example = item.example || fallbackExample(progress.current.en);
  if (example) {
    w.addSpacer(family === "small" ? 6 : 8);
    const ex = w.addText(example);
    ex.font = Font.italicSystemFont(exSize);
    ex.textColor = new Color("#9EC0DE");
    ex.lineLimit = family === "small" ? 3 : family === "large" ? 4 : 3;
    ex.minimumScaleFactor = 0.65;
  }
...
async function showHistory(progress) {
  ...
  const lines = entries.slice(0, 40).map((e) => {
    ...
    const n = e.shownCount ? " ×" + e.shownCount : "";
    return `${e.en}${ph}${n} — ${e.ru}`;
  });
  const a = new Alert();
  a.title = `Показано (${entries.length})`;
...
async function runMenu(progress) {
  ...
  alert.message =
    ...
    phoneticLine +
    exampleLine +
    "\n\nСлово само меняется каждые " +
    ROTATE_HOURS +
    " ч.";

  alert.addAction("⏭ Следующее сейчас");
  alert.addAction("🔊 Произнести");
  alert.addAction("📜 История");
  alert.addDestructiveAction("Сбросить прогресс");
  ...
  if (choice === 0) {
    ...
    conf.message = "Очередь начнётся с начала.";
    conf.addDestructiveAction("Сбросить");
    conf.addCancelAction("Отмена");
...
async function main() {
  ...
  progress = ensureCurrentWord(loadProgress());
