// WordOfDayCore — пассивный виджет v4: связанные слова + подвал N/total | timer
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V4";
...
function nextRefreshDate(progress) {
  // чаще обновляем, чтобы таймер в подвале был живым
  const shownAt = progress.shownAt || nowMs();
  const due = shownAt + rotateMs();
  const in15 = nowMs() + 15 * 60 * 1000;
  return new Date(Math.min(due, in15));
}

function countdownLabel(progress) {
  const shownAt = progress.shownAt || nowMs();
  let leftMs = Math.max(0, shownAt + rotateMs() - nowMs());
...
function footerLabel(progress) {
  const num =
    progress.current && typeof progress.current.index === "number"
      ? progress.current.index + 1
      : 1;
  const total = WORDS.length;
  return num + "/" + total + "  |  " + countdownLabel(progress);
}
...
/** Связанные по форме EN; иначе похожие по смыслу (пересечения RU). */
function findRelated(current, limit) {
  ...
  // если мало — соседние по списку (мягкий запасной вариант)
...
function createWidget(progress) {
  ...
  const ruSize = family === "large" ? 14 : family === "small" ? 11 : 12;
  const relEn = family === "large" ? 14 : family === "small" ? 11 : 12;
  const relRu = family === "large" ? 12 : family === "small" ? 10 : 11;
  ...
  ru.textColor = new Color("#FFFFFF", 0.88);
  ru.lineLimit = 2;
  ...
  if (related.length) {
    ...
    for (let i = 0; i < related.length; i++) {
      ...
      const line = w.addText(r.en);
      line.font = Font.semiboldSystemFont(relEn);
      line.textColor = new Color("#FFFFFF", 0.78);
      line.lineLimit = 1;
      line.minimumScaleFactor = 0.6;
      addShadow(line, 0.4);

      const lineRu = w.addText(r.ru);
      lineRu.font = Font.systemFont(relRu);
      lineRu.textColor = new Color("#FFFFFF", 0.58);
      lineRu.lineLimit = 1;
      lineRu.minimumScaleFactor = 0.65;
      addShadow(lineRu, 0.35);
    }
  }

  w.addSpacer();

  const foot = w.addText(footerLabel(progress));
...
async function runMenu(progress) {
  const related = findRelated(progress.current, 3);
  const relLine = related.length
    ? "\n\nRelated:\n" + related.map((r) => r.en + " - " + r.ru).join("\n")
    : "";
