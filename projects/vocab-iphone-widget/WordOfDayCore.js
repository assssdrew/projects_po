// WordOfDayCore — логика виджета (обновляется сама через bootstrap)
// Не открывай этот файл руками — его пишет скрипт WordOfDay
...
function createWidget(progress) {
  ...
  en.font = Font.boldSystemFont(18);
...
async function afterMark(progress) {
  const key = progress.current.en.toLowerCase();
  const item = progress.history[key];
  if (isComplete(item)) {
    ...
    const miss = [];
    if (!item.seen) miss.push("увидел");
    if (!item.learned) miss.push("выучил");
    if (!item.reviewed) miss.push("повторил");
    await showToast("Отмечено.\nЕщё нужно: " + miss.join(", "));
  }
}
...
async function runApp(progress) {
  ...
  if (choice === 0) {
    ...
    if ((await conf.presentAlert()) === 0) {
      ...
      await showToast("Прогресс сброшен");
    }
  }
}
