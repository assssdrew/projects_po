// WordOfDayCore — v39: small=EN слова, medium=как сейчас, large=interview Q&A
...
function reshuffleInterviews(progress) {
  progress.interviewOrder = shuffleIndices(INTERVIEWS.length);
...
function addInterviewBlock(w, card, family) {
  const maxW = contentWidth();
  const qSize = family === "extraLarge" ? 22 : 20;
  const aSize = family === "extraLarge" ? 16 : 15;
  const ruSize = Math.max(12, aSize - 3);
  const q = (card && card.q) || "";
  const qRu = (card && card.qRu) || "";
  const a = (card && card.a) || "";

  const qLines = wrapLines(q, qSize, Math.max(40, maxW - 2), 3);
  const aLines = wrapLines(a, aSize, Math.max(40, maxW - 2), 6);
  const ruLines = qRu
    ? wrapLines(qRu, ruSize, Math.max(40, maxW - 2), 2)
    : [];
  const total =
    (qLines.length || 1) + ruLines.length + (aLines.length || 1);
  const sat = Math.max(0, Math.min(1, (total - 5) / 6));
  const spreadFlex = sat < 0.45;
  const gap = gapForSaturation(sat, family);

  function between() {
    if (spreadFlex) w.addSpacer();
    else w.addSpacer(Math.max(gap, 6));
  }

  const qUse = qLines.length ? qLines : [q];
  for (let i = 0; i < qUse.length; i++) {
    const t = leftText(w.addText(qUse[i]));
    t.font = Font.boldSystemFont(qSize);
    t.textColor = enColor();
    t.lineLimit = 1;
    t.minimumScaleFactor = 1;
    addShadow(t, 0.5);
  ...
  if (ruLines.length) {
    ...
    for (let i = 0; i < ruLines.length; i++) {
      const t = leftText(w.addText(ruLines[i]));
      t.font = Font.systemFont(ruSize);
      t.textColor = ruColor(0.75);
      t.lineLimit = 1;
      t.minimumScaleFactor = 1;
      addShadow(t, 0.35);
    }
  ...
  between();
  const aUse = aLines.length ? aLines : [a];
  for (let i = 0; i < aUse.length; i++) {
    const t = leftText(w.addText(aUse[i]));
    t.font = Font.systemFont(aSize);
    t.textColor = ruColor(0.92);
    t.lineLimit = 1;
    t.minimumScaleFactor = 1;
    addShadow(t, 0.4);
  }
...
function createWidget(progress) {
  ...
  const padY =
    large || family === "small" || progress.kind === "rule" ? 12 : 16;
