// WordOfDayCore — v8: без подвала, смена 10с, EN фикс. размера
...
/** Минимальный интервал смены слова */
const ROTATE_SECONDS = 10;
const PASSIVE_MARKER = "PASSIVE_WIDGET_V8";
...
function nextRefreshDate() {
  // просим iOS обновить как можно скорее (система может ограничить)
...
function isLongPair(word, family) {
  const en = String(word.en || "");
  const ru = String(word.ru || "");
  const enLimit = family === "small" ? 10 : family === "large" ? 18 : 14;
  const totalLimit = family === "small" ? 20 : family === "large" ? 36 : 28;
  return en.length > enLimit || en.length + ru.length > totalLimit;
}

/**
 * EN всегда фиксированного размера (без уменьшения).
 * Если длинно — перевод на следующей строке.
 */
function addWordBlock(w, word, opts) {
  const enSize = opts.enSize;
  const ruSize = opts.ruSize;
  const ruAlpha = opts.ruAlpha;
  const stacked = isLongPair(word, opts.family);
...
function createWidget(progress) {
  ...
  // Центрирование по высоте, без подвала
  w.addSpacer();
