// WordOfDayCore — v10: мягкий перенос по реальной ширине medium
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V10";
...
/** Ширина medium/small/large по экрану устройства (pt). */
...
function contentWidth() {
  // padding 16+16, небольшой запас под тень
  return widgetOuterWidth() - 32;
}

/** Более мягкая оценка ширины — раньше переносили слишком рано. */
function estimateWidth(text, fontSize, bold) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    let factor;
    if (ch === " " || ch === "-") factor = 0.28;
    else if (code < 128) {
      if ("ilI1|'.,:;!".indexOf(ch) !== -1) factor = 0.3;
      else if ("mwMW@".indexOf(ch) !== -1) factor = 0.72;
      else factor = bold ? 0.52 : 0.46;
    } else {
      // кириллица
...
/** Перенос только если реально не влезает (с запасом в пользу одной строки). */
function needsWrap(word, enSize, ruSize) {
  ...
  // переносим только при явном переполнении (~8%)
  return line > max * 1.08;
}
