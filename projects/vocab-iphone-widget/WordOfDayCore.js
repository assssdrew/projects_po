// WordOfDayCore — v32: бронь 2-й строки перевода, +2 pt слева и между примерами
...
const PASSIVE_MARKER = "PASSIVE_WIDGET_V32";
...
function measureTextWidth(text, fontSize, bold) {
  const s = String(text || "");
  if (!s) return 0;
  // кириллица чуть шире латиницы — оценка с запасом, чтобы перенос срабатывал раньше
  const approx = s.length * fontSize * (bold ? 0.62 : 0.58);
  let exact = 0;
  try {
...
function addExampleBlock(w, word, opts) {
  ...
  let lines = accessory
    ? [ruText]
    : wrapLines(ruText, ruSize, maxW * 0.86, 2);
  if (!lines.length) lines = [ruText];
  ...
  // всегда место под 2 строки перевода (даже если влезло в одну)
  if (!accessory && lines.length < 2) {
    w.addSpacer(Math.ceil(ruSize * 1.15));
  }
}
...
function createWidget(progress) {
  ...
  const padY = 12;
  const padX = 12;
  const padLeft = padX + 2; // +2 pt от левого края
  ...
  if (accessory) {
    ...
    w.setPadding(padY, padLeft, padY, padX);
  }
  ...
  let blockGap = progress.kind === "rule" ? 14 : 10;
  ...
  if (progress.kind === "rule" && progress.rule) {
    ...
    if (family === "accessoryCircular") {
      ...
      // Три примера: EN + до 2 строк перевода; отступы 12 pt; зазор между блоками
      const items = [main].concat(related);
      const n = items.length;
      const slotW = contentWidth();
      const gap = accessory ? 2 : blockGap;
  ...
  // свободная высота вниз — текст прижат к верху (отступ padY)
