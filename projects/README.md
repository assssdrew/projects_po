# Projects

Рабочие проекты и артефакты.

## Виджеты словаря на iPhone

Два **разных** продукта. Не смешивать код, словари и ассеты.

| Папка | Для кого | Стек | Контент |
|-------|----------|------|---------|
| [`vocab-iphone-widget/`](vocab-iphone-widget/) | взрослый / экспат | Scriptable (JS) | ~948 слов |
| [`english-word-widget-kids/`](english-word-widget-kids/) | ребёнок ~10 лет | SwiftUI + WidgetKit | 300 слов + картинки, 30×10 |

Общее только идея: слово всегда на виду на Home Screen.
